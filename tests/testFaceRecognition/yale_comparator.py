...
def create_comparison_matrix():
    ...
    for file_name in faces:
        ...
        if subject_type == 'normal':
            normal_list[int_subject_number - 1 ] = file_name                       #os.path.join(path, file_name)
        else: 
            not_normal_list[int_subject_number - 1].append(file_name)             #os.path.join(path, file_name))
